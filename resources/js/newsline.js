...
if (filter === 'news'){
  $('.newsline-header').html('Новости');
} else if (filter === 'userposts'){
  $('.newsline-header').html('Записи друзей');
}else if (filter === 'threads'){
  $('.newsline-header').html('Избранное');
}
getNews(filter);
...
$('#compmode-btn').click(function(){
  if ( $('#compact').attr('value') == 'inactive'){
    $('#compact').prop('checked', true);
    $('#compact').attr('value','active');
    $('.replies').hide();
  } else {
    $('#compact').prop('checked', false);
    $('#compact').attr('value','inactive');
    $('.replies').show();
  }
});

function manageCompactMode(){
  if ( $('#compact').attr('value') == 'inactive'){
    $('.replies').show();
  } else {
    $('.replies').hide();
        ...
        if (result.content.length != 0){
          ...
          if ( result.content.length != 0){
            ...
            for (let i = 0; i < result.content.length; i++){
              ...
              if (media != 'nopic'){
                opPic = '\
                <div class="flex w-[25%] items-start mb-1">\
            ...
            $('.newsline-posts').html('\
            <span class="w-full text-center text-lg block mt-[20%]">Похоже, новостей пока нет</span>\
            ');
          }

        }
        else if (result.type === 'threads'){
          if ( result.content.length != 0){
            ...
            for (let i = 0; i < result.content.length; i++){
              ...
              if (user_replies.length != 0){
                ...
                for (let k = 0; k < user_replies.length && k < 3; k++){
                  var reply = user_replies[k];
                  reply_block += '\
                  <a href="/forum/'+ thread.catlink +'/'+ thread.id +'#'+ reply.id +'">\
                  <div id="'+ reply.id +'" class="reply_'+ reply.id +' flex-col w-[95%] text-white px-3 pt-2 pb-3 border-red-900/50 ml-auto border-l-[1px] border-b-[1px] rounded-md reply">\
                  <div class="w-full text-neutral-500 mb-1">\
                  <span>Ответ#'+ reply.id +'</span>\
                  <span>'+ new Date(reply.created_at).toLocaleString() +'</span>\
              ...
              $('.newsline-posts').append('\
              <div class="flex-col w-full text-white px-3 pt-2 rounded-md border-r-[2px] border-b-[2px] border-teal-600/50 bg-gray-900/25">\
              <div class="flex flex-row w-full text-neutral-500 mb-2">\
              <span>Тред#'+ thread.id +' от <a href="/profile/'+ thread.user_id +'">'+ thread.username +'</a></span>\
              <span>&nbsp'+ new Date(thread.created_at).toLocaleString() +'</span>\
              <span class="ml-2 underline">Ответы: '+ thread.replies +'</span>\
              <span class="ml-auto"> '+ thread.catname +'</span>\
              </div>\
              <div class="w-full flex flex-row">\
              '+ opPic +'\
              <a class="w-full ml-2" href="/forum/'+ thread.catlink +'/'+ thread.id +'">\
              <div class="flex flex-col w-[99%]">\
              <span class="w-full flex text-[22px] font-semibold mb-1">'+ thread.subject +'</span>\
              <div class="flex w-full text-base mb-3">\
              '+ thread.content.replaceAll('\n','<br>').substring(0,312) +'\
            ...
            $('.newsline-posts').html('\
            <span class="w-full text-center text-lg block mt-[20%]">Похоже, Вы пока не добавили ни одного раздела в избранное</span>\
            ');
          }
